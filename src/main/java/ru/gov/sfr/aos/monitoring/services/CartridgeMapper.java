...
public class CartridgeMapper {
    ...
    public void saveCartridgeModel(CartridgeModelDTO dto) {
        ...
        if(list.size() == 0) {
            ...
            for(int i = 0; i < dto.getModelPrinters().size(); i++) {
                List<Model> findByName = modelPrinterRepo.findByName(dto.getModelPrinters().get(i));
                modelsPrinters.add(findByName.get(0));
            }
