...
public interface SystemBlockRepo extends ObjectBuingWithSerialAndInventaryRepo<SystemBlock>{
    ...
    @Override
    List<SystemBlock> findBySerialNumberIgnoreCaseAndArchivedFalse(String serialNumber);
    @Override
    List<SystemBlock> findByInventaryNumberIgnoreCaseAndArchivedFalse(String inventaryNumber);
    @Override
   ...
   + "WHERE (ob.archived = false) AND (((?1 is NULL or ?1 = '') or (sysblock.status = ?1)) "
