...
public interface ServerRepo extends ObjectBuingWithSerialAndInventaryRepo <Server> {
   ...
   @Query(value = "SELECT server.*, ob.*, server_model.* "
   + "FROM server server "
   ...
   + "AND ((?2 is NULL or ?2 = '') or (server_model.model = ?2)) "
   + "AND ((?3 is NULL or ?3 = '') or (server.year_created >= ?3)) "
   + "AND ((?4 is NULL or ?4 = '') or (server.year_created <= ?4)) ", nativeQuery = true)
    List<Server> findServerByAllFilters (String status, String model, String yearCreatedOne, String yearCreatedTwo);
